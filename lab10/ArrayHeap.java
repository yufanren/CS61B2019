...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void changePriorityHelper(int index, T item, double priority) {
        ...
        int rindex = rightIndex(index);
        if (inBounds(lindex)) {
            changePriorityHelper(lindex, item, priority);
            if (inBounds(rindex)) {
                changePriorityHelper(rindex, item, priority);
            }
        }
    }
    ...
    private void validateSinkSwimArg(int index) {
        ...
        if (index > size) {
            String s = "sink/swim called on " + String.valueOf(index) + ", size is " + String.valueOf(size) + ".";
